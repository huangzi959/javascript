...
!(async () => {
  ...
  return new Promise((resolve) => {
    ...
    const link =  `https://my.ruanmei.com/api/UserSign/sign?userHash=${userHash}&coinHistoryType=5&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
...
function qiyu(){
  return new Promise((resolve) => {
    const userHash = $.getdata($.userHash);
    const link =  `https://my.ruanmei.com/api/UserSign/sign?userHash=${userHash}&coinHistoryType=6&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
...
function app() {
  return new Promise((resolve) => {
    const userHash = $.getdata($.userHash);
    const link =  `https://my.ruanmei.com/api/UserSign/sign?userHash=${userHash}&type=0&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
        "User-Agent":
          "Mozilla/5.0 (Linux; Android 10; Redmi K30 5G Build/QKQ1.191222.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/83.0.4103.101 Mobile Safari/537.36 ithome/rmsdklevel2/day/7.37",
          'Referer': 'https://my.ruanmei.com/app/user/signin.html?hidemenu=1&appver=2'
      },
    };
    $.get(url, (err, resp, data) => {
...
function miniProgram() {
  return new Promise((resolve) => {
    const userHash = $.getdata($.userHash);
    const link =  `https://my.ruanmei.com/api/UserSign/WeChatSign?userHash=${userHash}&coinHistoryType=157&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
        "User-Agent":
          "Mozilla/5.0 (iPhone; CPU iPhone OS 15_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.18(0x1800122c) NetType/WIFI Language/zh_CN miniProgram/wx8869812e2ddf7133",
      },
    };
    $.get(url, (err, resp, data) => {
      try {
...
function Zuihuimai() {
  return new Promise((resolve) => {
    const userHash = $.getdata($.userHash);
    const link =  `https://my.ruanmei.com/api/UserSign/sign?userHash=${userHash}&type=3&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
        "User-Agent":
          "Zuihuimai/1.42 (iPhone; iOS 15.2; Scale/3.00)",
      },
    };
    $.get(url, (err, resp, data) => {
      try {
        const res = JSON.parse(data);
        console.log("最会买:"+data+"\n\n")
...
function WeChatSign() {
  return new Promise((resolve) => {
    const userHash = $.getdata($.userHash);
    const link =  `https://my.ruanmei.com/api/UserSign/WeChatSign?userHash=${userHash}&coinHistoryType=158&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
        "User-Agent":
          "Mozilla/5.0 (iPhone; CPU iPhone OS 15_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.18(0x1800122f) NetType/WIFI Language/zh_CN miniProgram/wx8869812e2ddf7133",
      },
    };
    $.get(url, (err, resp, data) => {
      try {
        const res = JSON.parse(data);
        console.log("公众号:"+data+"\n\n")
      } catch (e) {
        $.logErr(e, resp);
      } finally {
        resolve();
...
function LaPin() {
  return new Promise((resolve) => {
    const userHash = $.getdata($.userHash);
    const link =  `https://my.ruanmei.com/api/usersign/sign?userHash=${userHash}&type=1&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
        "User-Agent":
          "LaPin/3.92 (iPhone; iOS 15.2; Scale/3.00)",
          'Referer': 'https://my.ruanmei.com/app/user/signin.html?hidemenu=1&appver=2'
      },
    };
    $.get(url, (err, resp, data) => {
...
function yunrili() {
  return new Promise((resolve) => {
    const userHash = $.getdata($.userHash);
    const link =  `https://my.ruanmei.com/api/usersign/yunrilisign?userHash=${userHash}&coinHistoryType=160&appver=3.03&platform=windows&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
...
// 云日历安卓
function yunriliAndroid() {
  return new Promise((resolve) => {
    const userHash = $.getdata($.userHash);
    const link =  `https://my.ruanmei.com/api/usersign/yunrilisign?userHash=${userHash}&coinHistoryType=159&timestamp=${now}&${gnekk()}=${geKsk()}`;
    const url = {
      url: link,
      headers: {
        Host: "my.ruanmei.com",
...
function encrypt(str) {
  ...
  return $.CryptoJS.enc.Base64.parse(
    $.CryptoJS.DES.encrypt(str, keyHex, {
...
function gekk(S) {
  ...
  const timeStemp = new Date(now).getTime();
  const today = new Date(now).getDate();
  const Value1 = Math.round(timeStemp / 50000) * today * 3;
  if (S === 3) {
    return (
      Skey[parseInt((Value1 % 10000) / 1000) * today] +
      Skey[parseInt((Value1 % 1000) / 100) * today] +
      Skey[parseInt((Value1 % 100) / 10) * today]
    );
  } else if (S === 8) {
    return (
      Skey[parseInt((Value1 % 100000000) / 10000000) * today] +
      Skey[parseInt((Value1 % 10000000) / 1000000) * today] +
      Skey[parseInt((Value1 % 1000000) / 100000) * today] +
      Skey[parseInt((Value1 % 100000) / 10000) * today] +
      Skey[parseInt((Value1 % 10000) / 1000) * today] +
      Skey[parseInt((Value1 % 1000) / 100) * today] +
      Skey[parseInt((Value1 % 100) / 10) * today] +
      Skey[parseInt(Value1 % 10) * today]
    );
  }
}
